...
public:
	...
	uint32_t maxBytes;
	int maxPackages;
	std::string filename = "Experiment_1.csv";
...
main(int argc, char **argv)
{
	...
	for(int i = 0; i < 7; i++){
		...
		highwayExperiment.vel1 = highwayExperiment.vel1 + 5;
		highwayExperiment.vel2 = highwayExperiment.vel2 + 5;
	}
...
HighwayExperiment::runExperiment (float gap){
	...
	//Getting stats from the flow monitor
		if ((monitor.sourceAddress== interface[0].GetAddress(0) && monitor.destinationAddress == interface[CONECTIONS - 1].GetAddress(1))){
			...
			outputFile << vel1 << ","<< throughput << ","<< PDR << std::endl;
			outputFile.close();
		}
	}
