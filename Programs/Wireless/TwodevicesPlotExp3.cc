...
public:
	...
	uint32_t maxBytes;
	int maxPackages;
	std::string filename = "Experiment_3.csv";
...
main(int argc, char **argv)
{
	...
	highwayExperiment.gap = 60.0; 
	highwayExperiment.vel1 = 0.0;
	highwayExperiment.vel2 = 0.0;
	highwayExperiment.vel3 = 0.0;
	highwayExperiment.vel4 = 0.0;
	highwayExperiment.vel5 = 0.0;
	highwayExperiment.vel6 = 0.0;

	highwayExperiment.thLast = 0.0;
	highwayExperiment.pdrLast = 1.0;
	...
	//highwayExperiment.runExperiment(5);
	for(int hops = 2; hops < 7; hops++){
		highwayExperiment.runExperiment(hops);
	}
...
HighwayExperiment::Configure (int argc, char **argv)
{
	...
	senderStop = simTime - 1.0;//sec
	receiverStop = simTime + 1.0;//sec
...
void 
HighwayExperiment::runExperiment (int hops){
	int nNodes = 6;
	...
	int link = 0;
	address[link].SetBase("10.1.1.0", "255.255.255.0"); 
	interface[link] = address[link].Assign(devices[link]); 

	link = 1;
	address[link].SetBase("10.1.2.0", "255.255.255.0"); 
	interface[link] = address[link].Assign(devices[link]); 

	link = 2;
	address[link].SetBase("10.1.3.0", "255.255.255.0"); 
	interface[link] = address[link].Assign(devices[link]); 

	link = 3;
	address[link].SetBase("10.1.4.0", "255.255.255.0"); 
	interface[link] = address[link].Assign(devices[link]); 

	link = 4;
	address[link].SetBase("10.1.5.0", "255.255.255.0"); 
	interface[link] = address[link].Assign(devices[link]); 
	...
	for(int i = 1; i < nNodes ; i ++){
		positionAlloc->Add(Vector(i*gap,0.0, 0.0)); 
	}
	...
	Ptr<ConstantVelocityMobilityModel> mob3 = nodes.Get(2) -> GetObject<ConstantVelocityMobilityModel>();
	mob3->SetVelocity(Vector(vel3, 0, 0));

	Ptr<ConstantVelocityMobilityModel> mob4 = nodes.Get(3) -> GetObject<ConstantVelocityMobilityModel>();
	mob3->SetVelocity(Vector(vel4, 0, 0));

	Ptr<ConstantVelocityMobilityModel> mob5 = nodes.Get(4) -> GetObject<ConstantVelocityMobilityModel>();
	mob3->SetVelocity(Vector(vel5, 0, 0));

	Ptr<ConstantVelocityMobilityModel> mob6 = nodes.Get(5) -> GetObject<ConstantVelocityMobilityModel>();
	mob3->SetVelocity(Vector(vel6, 0, 0));
	...
	AddressValue remoteAddress(InetSocketAddress(interface[hops - 2].GetAddress(1), SERVER));  //
	...
	ApplicationContainer receiverApp = sinkHelper.Install(nodes.Get(hops - 1)); //Reciver is the las node

	receiverApp.Start(Seconds(receiverStart));
	receiverApp.Stop(Seconds(receiverStop));

	
	// V4TraceRouteHelper traceroute("10.1.5.2"); // Objetive of the trace 
	...
	// anim.SetMaxPktsPerTraceFile(maxPackages + 1);
	...
	//Getting stats from the flow monitor
		if ((monitor.sourceAddress== interface[0].GetAddress(0) && monitor.destinationAddress == interface[hops - 2].GetAddress(1))){
			...
			//std::cout << "vel 1:   " << vel1 << " vel 2:  " << vel2 << " vel 3:  " << vel3  << "\n";


			
			outputFile << hops << ","<< throughput << ","<< PDR << ","<< (statsPointer->second.delaySum)/(statsPointer->second.rxPackets) << std::endl;
			outputFile.close();
