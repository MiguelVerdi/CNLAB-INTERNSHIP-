...
- Each of the `InputFiles/highway*_PQGR.mat` files is processed in Matlab Online using the file named `ExperimentmToCSV.m`, you only have to copy paste the code in matlab and charge the file after running the program, it would ask for the file, this generates the files found in `MatlabProcessingResult` which are `MatlabProcessingResult/highway*_PQGR.csv` files, containing the data from the .mat file but unpacked so that C++ can read them in a more orderly.  
